...
var outerDia = 260;
var circles = [];
for(let i = 0; i < 4; i++)circles.push(new _paperDefault.default.Path.Circle(_paperDefault.default.view.center, outerDia - 60 * i));
circles[0].fillColor = 'red';
circles[1].fillColor = 'white';
circles[2].fillColor = 'red';
circles[3].fillColor = 'blue';
//set up star
var star = new _paperDefault.default.Path.Star(_paperDefault.default.view.center, 5, outerDia - 180, outerDia - 228);
star.fillColor = 'white';

},{"paper":"2aK0U","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"2aK0U":[function(require,module,exports) {
/*!
...
},{"./node/self.js":"joWnX","acorn":"a2LDj","./node/extend.js":"joWnX"}],"joWnX":[function(require,module,exports) {
"use strict";

},{}],"a2LDj":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
function isInAstralSet(code, set) {
    var pos = 65536;
    for(var i1 = 0; i1 < set.length; i1 += 2){
        pos += set[i1];
...
function nextLineBreak(code, from, end) {
    ...
    for(var i2 = from; i2 < end; i2++){
        ...
        if (isNewLine(next)) return i2 < end - 1 && next === 13 && code.charCodeAt(i2 + 1) === 10 ? i2 + 2 : i2 + 1;
    }
    return -1;
}
var nonASCIIwhitespace = /[\u1680\u2000-\u200a\u202f\u205f\u3000\ufeff]/;
...
function getOptions(opts) {
    ...
    if (options.ecmaVersion === "latest") options.ecmaVersion = 100000000;
...
prototypeAccessors.allowSuper.get = function() {
    var ref1 = this.currentThisScope();
    var flags = ref1.flags;
    var inClassFieldInit = ref1.inClassFieldInit;
...
Parser.extend = function extend() {
    ...
    var cls = this;
    for(var i4 = 0; i4 < plugins.length; i4++)cls = plugins[i4](cls);
    return cls;
};
...
pp$8.parseTopLevel = function(node) {
    ...
    if (this.inModule) for(var i5 = 0, list1 = Object.keys(this.undefinedExports); i5 < list1.length; i5 += 1){
        var name = list1[i5];
...
pp$8.isLet = function(context) {
    ...
    if (nextCh === 91 || nextCh === 92 || nextCh > 55295 && nextCh < 56320) return true;
     // '[', '/', astral
...
pp$8.parseBreakContinueStatement = function(node, keyword) {
    ...
    // continue to.
    var i6 = 0;
    for(; i6 < this.labels.length; ++i6){
        var lab = this.labels[i6];
        if (node.label == null || lab.name === node.label.name) {
            if (lab.kind != null && (isBreak || lab.kind === "loop")) break;
            if (node.label && isBreak) break;
        }
    }
    if (i6 === this.labels.length) this.raise(node.start, "Unsyntactic " + keyword);
...
pp$8.parseLabeledStatement = function(node, maybeName, expr, context) {
    ...
    for(var i7 = this.labels.length - 1; i7 >= 0; i7--){
        var label$1 = this.labels[i7];
...
pp$8.parseClassElement = function(constructorAllowsSuper) {
    ...
    var ecmaVersion1 = this.options.ecmaVersion;
    ...
    if (this.eatContextual("static")) {
        ...
        if (ecmaVersion1 >= 13 && this.eat(types$1.braceL)) {
    ...
    if (!keyName && ecmaVersion1 >= 8 && this.eatContextual("async")) {
        if ((this.isClassElementNameStart() || this.type === types$1.star) && !this.canInsertSemicolon()) isAsync = true;
        else keyName = "async";
    }
    if (!keyName && (ecmaVersion1 >= 9 || !isAsync) && this.eat(types$1.star)) isGenerator = true;
    ...
    // Parse element value
    if (ecmaVersion1 < 13 || this.type === types$1.parenL || kind !== "method" || isGenerator || isAsync) {
...
pp$8.exitClassBody = function() {
    ...
    for(var i8 = 0; i8 < used.length; ++i8){
        var id = used[i8];
...
pp$8.parseExport = function(node, exports) {
    ...
    if (this.shouldParseExportStatement()) {
        ...
        if (this.eatContextual("from")) {
            ...
            for(var i9 = 0, list3 = node.specifiers; i9 < list3.length; i9 += 1){
...
pp$8.checkPatternExport = function(exports, pat) {
    ...
    else if (type === "ObjectPattern") for(var i10 = 0, list4 = pat.properties; i10 < list4.length; i10 += 1){
        var prop = list4[i10];
...
pp$8.checkVariableExport = function(exports, decls) {
    if (!exports) return;
    for(var i11 = 0, list5 = decls; i11 < list5.length; i11 += 1){
        var decl = list5[i11];
...
pp$8.adaptDirectivePrologue = function(statements) {
    for(var i12 = 0; i12 < statements.length && this.isDirectiveCandidate(statements[i12]); ++i12)statements[i12].directive = statements[i12].expression.raw.slice(1, -1);
};
...
pp$7.toAssignable = function(node, isBinding, refDestructuringErrors) {
    if (this.options.ecmaVersion >= 6 && node) switch(node.type){
        ...
        case "ObjectExpression":
            ...
            for(var i13 = 0, list6 = node.properties; i13 < list6.length; i13 += 1){
                var prop = list6[i13];
...
pp$7.toAssignableList = function(exprList, isBinding) {
    ...
    for(var i14 = 0; i14 < end; i14++){
        var elt = exprList[i14];
...
pp$7.checkLValPattern = function(expr, bindingType, checkClashes) {
    ...
    switch(expr.type){
        case "ObjectPattern":
            for(var i15 = 0, list7 = expr.properties; i15 < list7.length; i15 += 1){
                var prop = list7[i15];
...
pp$6.inGeneratorContext = function() {
    for(var i16 = this.context.length - 1; i16 >= 1; i16--){
        var context = this.context[i16];
...
pp$5.parseTemplateElement = function(ref4) {
...
pp$5.parseTemplate = function(ref5) {
    if (ref5 === void 0) ref5 = {
    };
    var isTagged = ref5.isTagged;
...
pp$5.isSimpleParamList = function(params) {
    for(var i17 = 0, list8 = params; i17 < list8.length; i17 += 1){
        var param = list8[i17];
...
pp$5.checkParams = function(node, allowDuplicates) {
    ...
    for(var i18 = 0, list9 = node.params; i18 < list9.length; i18 += 1){
        var param = list9[i18];
...
pp$5.checkUnreserved = function(ref6) {
    var start = ref6.start;
    var end = ref6.end;
    var name = ref6.name;
...
pp$3.declareName = function(name, bindingType, pos) {
    ...
    } else for(var i19 = this.scopeStack.length - 1; i19 >= 0; --i19){
        var scope$3 = this.scopeStack[i19];
...
pp$3.currentVarScope = function() {
    for(var i20 = this.scopeStack.length - 1;; i20--){
        var scope = this.scopeStack[i20];
        if (scope.flags & SCOPE_VAR) return scope;
    }
};
// Could be useful for `this`, `new.target`, `super()`, `super.property`, and `super[property]`.
pp$3.currentThisScope = function() {
    for(var i21 = this.scopeStack.length - 1;; i21--){
        var scope = this.scopeStack[i21];
...
RegExpValidationState.prototype.at = function at(i22, forceU) {
    if (forceU === void 0) forceU = false;
    var s = this.source;
    var l = s.length;
    if (i22 >= l) return -1;
    var c = s.charCodeAt(i22);
    if (!(forceU || this.switchU) || c <= 55295 || c >= 57344 || i22 + 1 >= l) return c;
    var next = s.charCodeAt(i22 + 1);
    return next >= 56320 && next <= 57343 ? (c << 10) + next - 56613888 : c;
};
RegExpValidationState.prototype.nextIndex = function nextIndex(i23, forceU) {
    if (forceU === void 0) forceU = false;
    var s = this.source;
    var l = s.length;
    if (i23 >= l) return l;
    var c = s.charCodeAt(i23), next;
    if (!(forceU || this.switchU) || c <= 55295 || c >= 57344 || i23 + 1 >= l || (next = s.charCodeAt(i23 + 1)) < 56320 || next > 57343) return i23 + 1;
    return i23 + 2;
};
 ...
 */ pp$1.validateRegExpFlags = function(state) {
    ...
    for(var i24 = 0; i24 < flags.length; i24++){
        var flag = flags.charAt(i24);
        if (validFlags.indexOf(flag) === -1) this.raise(state.start, "Invalid regular expression flag");
        if (flags.indexOf(flag, i24 + 1) > -1) this.raise(state.start, "Duplicate regular expression flag");
    }
};
/**
...
pp$1.regexp_pattern = function(state) {
    ...
    for(var i25 = 0, list10 = state.backReferenceNames; i25 < list10.length; i25 += 1){
        var name = list10[i25];
...
pp$1.regexp_eatRegExpIdentifierName = function(state) {
    ...
    if (this.regexp_eatRegExpIdentifierStart(state)) {
        ...
        while(this.regexp_eatRegExpIdentifierPart(state))state.lastStringValue += codePointToString$1(state.lastIntValue);
        return true;
    }
    return false;
};
...
pp.readInt = function(radix, len, maybeLegacyOctalNumericLiteral) {
    ...
    for(var i27 = 0, e = len == null ? Infinity : len; i27 < e; ++i27, ++this.pos){
        var code = this.input.charCodeAt(this.pos), val = void 0;
        if (allowSeparators && code === 95) {
            if (isLegacyOctalNumericLiteral) this.raiseRecoverable(this.pos, "Numeric separator is not allowed in legacy octal numeric literals");
            if (lastCode === 95) this.raiseRecoverable(this.pos, "Numeric separator must be exactly one underscore");
            if (i27 === 0) this.raiseRecoverable(this.pos, "Numeric separator is not allowed at the first of digits");
            lastCode = code;
            continue;
        }
        if (code >= 97) val = code - 97 + 10;
...
},{}]},["j1F46","hD4hw"], "hD4hw", "parcelRequire8173")
